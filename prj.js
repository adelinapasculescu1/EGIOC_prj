...
function initEdges() {
    ...
    var sidematerial = new THREE.MeshPhongMaterial({ color: 0xffffff, specular: 0x333333, shininess: 5 });
    ...
    var backbox = new THREE.Mesh(new THREE.BoxGeometry(400, 400, 1), sidematerial);
...
function drawText(msg) {
    ...
    text.style.left = window.innerWidth / 2.25 + "px";
    text.innerHTML = msg;
...
function start() {
    ...
    function mainLoop(timestamp) {
        ...
        if ((paddle.dir === -1 && paddle.x > paddleDelta) || (paddle.dir === 1 && paddle.x + paddleDelta + paddle.width < width))
...
function initKeys() {
    document.addEventListener("keydown", function(e) {
        if (e.key === "ArrowRight")
            ...
            paddle.dir = -1;
        else if (e.key === " " && game.state === "ready") {
    ...
    document.addEventListener("keyup", function(e) {
        if ((e.key === "ArrowRight" && paddle.dir === 1) || (e.key === "ArrowLeft" && paddle.dir === -1))
            paddle.dir = 0;
    }, false);
}
...
function initGame() {
    ...
    for (var i = 0; i < ROWS; i++) {
        blocks[i] = [];
        for (var j = 0; j < COLS; j++) {
            var material = new THREE.MeshPhongMaterial({ color: new THREE.Color(randColor(), randColor(), randColor()) });
            var object = new THREE.Mesh(geometry, material);
            blocks[i][j] = { status: 0, object: object };
            object.position.set(j * blockWidth + blockWidth / 2, height - (i * blockHeight + blockHeight / 2), 0);
            scene.add(object);
        }
    }
    ...
    ball.mesh = new THREE.PointLight(0x000000, 2, 300);
